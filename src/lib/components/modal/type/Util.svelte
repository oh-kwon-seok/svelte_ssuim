...
<script>
    ...
    import {save} from '$lib/store/type/function';
    ...
    <Modal title={`품목분류 ${label_title}`} color={color} bind:open={$type_modal_state[title]['use']} size="xl" placement={title === 'add' || title === 'check_delete'  ? 'center' : 'center-right'}   class="w-full">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <Label class="space-y-2">
            <span>품목분류</span>
            <Input type="text" id="last_name" placeholder="품목분류를 입력하세요" required bind:value={$type_form_state['name']}/>
       ...
       {/if}
        ...
        <Button  color={title === 'add' || title === 'update'  ? 'blue' : 'red'}   class="w-full" on:click={save($type_form_state,title)}>{label_title}</Button>
