...
<script>
    ...
    <Modal title={`규격 ${label_title}`} color={color} bind:open={$standard_modal_state[title]['use']} size="xl" placement={title === 'add' || title === 'check_delete'  ? 'center' : 'center-right'}   class="w-full">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <Label class="space-y-2">
            <span>규격</span>
            <Input type="text" id="last_name" placeholder="규격을 입력하세요" required bind:value={$standard_form_state['name']}/>
            
            {#if $standard_form_state['name'] === '' && $common_alert_state['value'] === true}
          ...
          {#if $standard_modal_state['title'] === 'update'}
            <Label class="space-y-2">
              <span>사용유무</span>
              <Select id="countries" class="mt-2" bind:value={$standard_form_state['used']} placeholder="">
