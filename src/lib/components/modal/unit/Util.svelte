...
<script>
    ...
    import {unit_modal_state, unit_form_state} from '$lib/store/unit/state';
    ...
    import {save} from '$lib/store/unit/function';
    ...
    <Modal title={`단위 ${label_title}`} color={color} bind:open={$unit_modal_state[title]['use']} size="xl" placement={title === 'add' || title === 'check_delete'  ? 'center' : 'center-right'}   class="w-full">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <Label class="space-y-2">
            <span>단위</span>
            <Input type="text" id="last_name" placeholder="단위를 입력하세요" required bind:value={$unit_form_state['name']}/>
            
            {#if $unit_form_state['name'] === '' && $common_alert_state['value'] === true}
            <Helper class="mt-2" color="red"><span class="font-medium">데이터를 입력해주세요</span></Helper>
            {/if}
          </Label>

    
          {#if $unit_modal_state['title'] === 'update'}
            <Label class="space-y-2">
              <span>사용유무</span>
              <Select id="countries" class="mt-2" bind:value={$unit_form_state['used']} placeholder="">
       ...
       {/if}
        ...
        <Button  color={title === 'add' || title === 'update'  ? 'blue' : 'red'}   class="w-full" on:click={save($unit_form_state,title)}>{label_title}</Button>
        {#if $common_alert_state['type'] === 'save' && $common_alert_state['value'] === true}
