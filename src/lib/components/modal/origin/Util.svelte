...
<script>
    ...
    import {origin_modal_state, origin_form_state} from '$lib/store/origin/state';
    ...
    import {save} from '$lib/store/origin/function';
    ...
    <Modal title={`원산지 ${label_title}`} color={color} bind:open={$origin_modal_state[title]['use']} size="xl" placement={title === 'add' || title === 'check_delete'  ? 'center' : 'center-right'}   class="w-full">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <Label class="space-y-2">
            <span>원산지</span>
            <Input type="text" id="last_name" placeholder="원산지를 입력하세요" required bind:value={$origin_form_state['name']}/>
            
            {#if $origin_form_state['name'] === '' && $common_alert_state['value'] === true}
            <Helper class="mt-2" color="red"><span class="font-medium">데이터를 입력해주세요</span></Helper>
            {/if}
          </Label>

          {#if $origin_modal_state['title'] === 'update'}
            <Label class="space-y-2">
              <span>사용유무</span>
              <Select id="countries" class="mt-2" bind:value={$origin_form_state['used']} placeholder="">
       ...
       {/if}
        ...
        <Button  color={title === 'add' || title === 'update'  ? 'blue' : 'red'}   class="w-full" on:click={save($origin_form_state,title)}>{label_title}</Button>
